...
set jobfile "\$WORK/experiments/3_work_precision_diagram/slurmjob.sh"
...
for ivp in $ivps
    echo cpu-long $ivp
    ssh slurm 'cd ~; sbatch -p cpu-long --gres=gpu:1 --exclude "slurm-v100-6" $jobfile "$ivp --gpu-nocheck"'
end
echo "Done"

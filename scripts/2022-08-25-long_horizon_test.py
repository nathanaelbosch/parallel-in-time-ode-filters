...
from pof.ivp import logistic, lotkavolterra, fitzhughnagumo
from pof.parallel_filtsmooth import linear_filtsmooth
from pof.sequential_filtsmooth import filtsmooth, extended_kalman_filter
...
def solve_parallel_eks(f, y0, ts, order, coarse_N=10):
    out, info = pof.solver.solve(
        f=f, y0=y0, ts=ts, order=order, init="constant", coarse_N=coarse_N
    )
...
def solve_sequential_eks(f, y0, ts, order):
    ...
    out, _ = extended_kalman_filter(x0, dtm, om)
...
# ivp = lotkavolterra(tmax=30)
ivp = fitzhughnagumo(tmax=1000)

dt = 1e-1
ts = jnp.arange(ivp.t0, ivp.tmax + dt, dt)
# ts_coarse = jnp.linspace(ts[0], ts[-1], 100)
ys_diffrax, *_ = solve_diffrax(ivp.f, ivp.y0, ts, diffrax.Dopri5())
ys_eks, _ = solve_sequential_eks(ivp.f, ivp.y0, ts, order=4)

ys_peks, info, _ = solve_parallel_eks(ivp.f, ivp.y0, ts, order=4)
info
# plt.plot(ys_peks)
